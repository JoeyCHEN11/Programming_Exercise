...
# add all geometries to the axis
...
def add_to_axis(geo, x = 0, y = 0):
    """
    add an object to axis, (x, y) for vector's origin point
    """
    if isinstance(geo, Point):
        if geo.is_vec:
            GEOAX.quiver([x], [y], [geo.x], [geo.y], scale_units='xy', scale=1, color='blue', width = 0.005)
        else:
            GEOAX.plot(geo.x, geo.y, marker='o', markersize=4, markerfacecolor='yellow', markeredgewidth=1, markeredgecolor='black', linestyle='None', label='point')
...
class Point:
    ...
    def rotated(self, d: float) -> 'Point':
        """get the point that rotated d (radian) counterclockwise around (0, 0)

        Returns:
            Point: rotated point
        """
        return Point(self.x * cos(d) - self.y * sin(d), self.y * cos(d) + self.x * sin(d))
...
def intersection(*args, **kwargs) -> bool:
    ...
    Returns:
        bool: True if they are intersection
    """
    if len(args) == 4:
        p1, p2, p3, p4 = args
...
def cross_points(*args, **kwargs) -> tuple[Point, Point]:
    """
    Circle and Line, or Circle and Circle.
    """
...
def andrew_scan(points: list[Point]) -> Polygon:
    ...
    Args:
        points (list[Point]): origin points

    Returns:
        Polygon: result polygon, it contains all other points, the on-segment points will also be contained
    """
    if(len(points) < 3):
...
if __name__ == '__main__':
    ...
    for i in range(q):
        ...
        p2 = Point(x3, y3)
        p3 = Point(x4, y4)
        ans = distance(Segment(p0,p1),Segment(p2,p3))
        add_geos([Segment(p0, p1), Segment(p2, p3)])
        print(f'{ans:.10f}')
    draw()