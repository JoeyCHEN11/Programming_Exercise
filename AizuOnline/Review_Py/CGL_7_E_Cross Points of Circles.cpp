...
int main(int argc, char const *argv[])
{
    ...
    auto cps = cross_points(c1, c2);
    if(cps.second < cps.first) swap(cps.first, cps.second);
    cout<<fixed<<setprecision(10)<<cps.first.x<<" "<<cps.first.y<<" "<<cps.second.x<<" "<<cps.second.y<<endl;
    //printf("%.10f %.10f\n",pro.x,pro.y);
