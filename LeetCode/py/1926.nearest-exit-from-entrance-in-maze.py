...
class Solution:
    def boundCheck(self, i: int, j: int) -> bool: 
        return 0 <= i < self.m and 0 <= j < self.n
    def isExist(self, i: int, j: int) -> bool:
        return i == self.m - 1 or i == 0 or j == 0 or j == self.n - 1 
    
    def bfs(self, ri: int, rj: int, maze: List[List[str]]) -> int:
        que = deque([(ri, rj)])
        self.distances[ri][rj] = 0
        while que:
            ui, uj = que.pop()

            for dx, dy in self.dir:
                vi, vj = ui + dx, uj + dy
                if(self.boundCheck(vi, vj) and maze[vi][vj] == '.' and self.distances[vi][vj] == -1):
                    self.distances[vi][vj] = self.distances[ui][uj] + 1
                    if self.isExist(vi, vj): 
                        return self.distances[vi][vj]
                    que.appendleft((vi, vj))
        return -1
        
    def nearestExit(self, maze: List[List[str]], entrance: List[int]) -> int:
        self.m, self.n = len(maze), len(maze[0])
        self.dir = [[0, -1], [-1, 0], [0, 1], [1, 0]]
        self.distances = [[-1] * self.n for _ in range(self.m)]
        return self.bfs(entrance[0], entrance[1], maze)
# @lc code=end
