/*
 * @lc app=leetcode id=406 lang=cpp
 *
 * [406] Queue Reconstruction by Height
 *
 * https://leetcode.com/problems/queue-reconstruction-by-height/description/
 *
 * algorithms
 * Medium (73.32%)
 * Likes:    6996
 * Dislikes: 709
 * Total Accepted:    296.6K
 * Total Submissions: 404.4K
 * Testcase Example:  '[[7,0],[4,4],[7,1],[5,0],[6,1],[5,2]]'
 *
 * You are given an array of people, people, which are the attributes of some
 * people in a queue (not necessarily in order). Each people[i] = [hi, ki]
 * represents the i^th person of height hi with exactly ki other people in
 * front who have a height greater than or equal to hi.
 * 
 * Reconstruct and return the queue that is represented by the input array
 * people. The returned queue should be formatted as an array queue, where
 * queue[j] = [hj, kj] is the attributes of the j^th person in the queue
 * (queue[0] is the person at the front of the queue).
 * 
 * 
 * Example 1:
 * 
 * 
 * Input: people = [[7,0],[4,4],[7,1],[5,0],[6,1],[5,2]]
 * Output: [[5,0],[7,0],[5,2],[6,1],[4,4],[7,1]]
 * Explanation:
 * Person 0 has height 5 with no other people taller or the same height in
 * front.
 * Person 1 has height 7 with no other people taller or the same height in
 * front.
 * Person 2 has height 5 with two persons taller or the same height in front,
 * which is person 0 and 1.
 * Person 3 has height 6 with one person taller or the same height in front,
 * which is person 1.
 * Person 4 has height 4 with four people taller or the same height in front,
 * which are people 0, 1, 2, and 3.
 * Person 5 has height 7 with one person taller or the same height in front,
 * which is person 1.
 * Hence [[5,0],[7,0],[5,2],[6,1],[4,4],[7,1]] is the reconstructed queue.
 * 
 * 
 * Example 2:
 * 
 * 
 * Input: people = [[6,0],[5,0],[4,0],[3,2],[2,2],[1,4]]
 * Output: [[4,0],[5,0],[2,2],[3,2],[1,4],[6,0]]
 * 
 * 
 * 
 * Constraints:
 * 
 * 
 * 1 <= people.length <= 2000
 * 0 <= hi <= 10^6
 * 0 <= ki < people.length
 * It is guaranteed that the queue can be reconstructed.
 * 
 * 
 */

// @lc code=start
#include <bits/stdc++.h>
using namespace std;

class Solution {
public:

    static bool compk (const vector<int>& p1, const vector<int>& p2) { return p1[1] < p2[1];}

    static bool compDec (const vector<int>& p1, const vector<int>& p2) { return p1[0] > p2[0] || (p1[0] == p2[0]) && p1[1] < p2[1]; }

    static bool compInc (const vector<int>& p1, const vector<int>& p2) { return p1[0] < p2[0] || (p1[0] == p2[0]) && p1[1] > p2[1]; }

    vector<vector<int>> reconstructQueue(vector<vector<int>>& people) {

        list<vector<int>> ans;

        // insert by k
        // sort(people.begin(), people.end(), compk);
        // for(auto& p : people){
        //     int k = p[1];
        //     auto pos = ans.begin();
        //     while(pos != ans.end()){
        //         if(pos->at(0) >= p[0]) {
        //             if(k > 0) k--;   
        //             else break;
        //         }
        //         pos++;
        //     }
        //     ans.insert(pos, p);
        // }

        //insert by height increasingly
        vector<vector<int>> ansinc (people.size());
        sort(people.begin(), people.end(), compInc);
        for(auto & p : people){
            int space = p[1] + 1;
            for(int pos = 0; pos < people.size(); pos++){
                if(ansinc[pos].empty()) {
                    space--;
                    if(!space){
                        ansinc[pos] = p;
                        break;
                    }
                }
            }
        }
        return ansinc;


        // insert by height (decreasingly)
        // sort(people.begin(), people.end(), compDec);
        // for(auto & p : people){
        //     auto pos = ans.begin();
        //     advance(pos, p[1]);
        //     ans.insert(pos, p);
        // }
        // return vector<vector<int>>(ans.begin(), ans.end());
    }
};
// @lc code=end

