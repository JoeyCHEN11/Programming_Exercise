...
public:
    int getNumberOfBacklogOrders(vector<vector<int>>& orders) {
        ...
        for(auto & order : orders){
            ...
            if(type == 0){
                while(!sells.empty() && amount > 0 && sells.top().first <= price){
                    auto [sellP, sellam] = sells.top(); sells.pop();

                    int matches = min(sellam, amount);
                    sellam -= matches;
                    amount -= matches;

                    if(sellam > 0) {
                        sells.emplace(sellP, sellam);
                    }
                }
                ...
                while(!buys.empty() && amount > 0 && buys.top().first >= price){
                    auto [buyP, buyam] = buys.top(); buys.pop();

                    int matches = min(buyam, amount);
                    buyam -= matches;
                    amount -= matches;

                    if(buyam > 0) {
                        buys.emplace(buyP, buyam);
                    }
                }
        ...
        int ans = 0;
        while(!buys.empty()){
            ans = (ans + buys.top().second % MOD) % MOD;
            
            buys.pop();
        }
        while(!sells.empty()){
            ans = (ans + sells.top().second % MOD) % MOD;
            sells.pop();
        }
        return ans;
    }
