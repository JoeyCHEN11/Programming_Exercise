...
public:
    int minOperationsMaxProfit(vector<int>& customers, int boardingCost, int runningCost) {
        ...
        int profit = 0, maxProfit = 0, rotate = -1;
        int i = 0, waitList = customers[0];
        while( i < n || waitList > 0){

            int board = min(waitList, 4);
            waitList -= board;
