...
public:
    ...
    vector<vector<int>> closestNodes(TreeNode* root, vector<int>& queries) {
        ans.resize(queries.size(), vector<int>(2, -1));
        inorder(root);
        for(int i = 0; i < queries.size(); i++){
            auto maxIter = lower_bound(nodes.begin(),nodes.end(), queries[i]);
            if(maxIter == nodes.end()) ans[i][0] = nodes.back();
            else {
                ans[i][1] = *maxIter;
                if(*maxIter == queries[i]) ans[i][0] = *maxIter;
                else if(maxIter != nodes.begin()) ans[i][0] = *(--maxIter);
            }
        }
        return ans;
    }
